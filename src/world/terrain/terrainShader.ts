...
export const TERRAIN_SURFACE_FRAGMENT = /* glsl */ `
  ...
  // Weathered rock: water streaks down the fall line, darker weathered
  // patches and pale lichen, all from one noise sample each (no banding: the
  // strata that used to be bent in here read as contour lines from afar).
  if (rockW > 0.01) {
    vec2 downhill = normalize(tN.xz + vec2(1e-4, 0.0));
    vec2 across = vec2(-downhill.y, downhill.x);
    float streak = smoothstep(0.5, 0.85, tNoise(vec2(dot(tPos.xz, across) * 0.45, tPos.y * 0.015)));
    float weathered = smoothstep(0.35, 0.75, tNoise(tPos.xz * 0.05 + tPos.y * 0.03));
    float lichen = smoothstep(0.62, 0.8, tNoise(tPos.xz * 0.35 + tPos.y * 0.2)) * (1.0 - streak);
    surfAlbedo *= 1.0 - rockW * (streak * 0.3 + weathered * 0.3);
    surfAlbedo = mix(surfAlbedo, vec3(0.42, 0.44, 0.3), rockW * lichen * 0.35);
  }
